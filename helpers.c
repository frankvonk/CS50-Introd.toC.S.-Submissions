...
int duration(string fraction)
{
    ...
    //Convert numerator to number of eighths
...
{
    //Gets octave from last char in note string
    int octave = note[strlen(note) - 1] - '0';

    // Determine Frequency of note in the given octive
    double freq = 0.0;

    // Determines note letter
    if (note[0] == 'A')
    {
        freq = 440;
    }
    ...
    // Returns frequency value as an int
    int result = round(freq);
    return result;
}
// Determine if a string represents a rest
